...
\optName{chimScoreMin}
  ...
  \optLine{Note: With --chimScoreUsePostStitch 1, this minimum is applied to post-stitch (final) chimeric scores in the new detection path.}
\optName{chimScoreDropMax}
  ...
  \optLine{Note: With --chimScoreUsePostStitch 1, this drop is evaluated against post-stitch chimeric scores; otherwise legacy gating uses pre-stitch scores for the initial threshold.}
...
\optName{chimScoreUsePostStitch}
  \optValue{0}
  \optLine{bool: 0/1. If 1, chimeric score drop/min thresholds are applied to post-stitch (final) chimeric scores; pre-stitch scores are only used for a permissive pre-filter.}

\optName{chimScorePreStitchAllowance}
  \optValue{32}
  \optLine{int{\textgreater}=0: allowance below the final threshold used for the pre-stitch permissive filter when --chimScoreUsePostStitch 1. Larger values increase compute; smaller values risk false negatives.}
